...
def create_character_animation(
    ...
    """
    Create animated SVG for a character

    Args:
        character_name: Name of the character
        animation_name: Name of the animation
        svg_code: Base SVG code
    ...
    try:
        ...
        if output_path is None:
            # Use asset manager to get correct path
            filename = f"{character_name.lower()}_{animation_name.lower()}.svg"
            output_path = str(asset_manager.get_path("animations", filename))
        else:
            # Convert to Path and ensure directories exist
            path_obj = Path(output_path)
            path_obj.parent.mkdir(parents=True, exist_ok=True)
            output_path = str(path_obj)

        svg_path = renderer.create_animation(
            character_name,
            animation_name,
            svg_code,
            output_path
        ...
        logger.info(f"Generated animation for {character_name}: {svg_path}")
        return svg_path
